...
export const EditorialWorkspaceModal: React.FC<EditorialWorkspaceModalProps> = ({
  ...
  // Portfolio item creation state (Admin only)
    ...
    >
      <div className="bg-[#fbf9f6] border border-[#e4e2df] rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-5 bg-white border-b border-[#e4e2df] flex items-center justify-between gap-4">
          <div>
            <span className="text-[11px] uppercase tracking-widest text-[#994524] font-semibold">
              {isAdmin ? 'Editorial Admin Console' : 'Client Collaboration Portal'}
            </span>
            <h2 className="font-serif text-2xl text-[#1b1c1a] font-medium">
              {isAdmin ? 'Inquiries & Live Portfolio Database' : 'Your Project Inquiries'}
            </h2>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {actionError && (
            <div className="p-3.5 rounded-lg bg-red-50 border border-red-200 text-xs text-red-700 flex items-center justify-between">
              ...
              <button
                ...
                className="text-red-800 font-semibold ml-4"
              >
                Dismiss
              </button>
            </div>
          ...
          {!user ? (
            <div className="py-12 px-6 bg-white rounded-xl border border-[#e4e2df] text-center max-w-lg mx-auto space-y-4">
              <span className="material-symbols-outlined text-4xl text-[#994524]">
                lock_person
              </span>
              ...
              <p className="text-sm text-[#55433c] leading-relaxed">
                Prospective clients can sign in with Google to track or update their submitted project inquiries. Administrator access ({'bajajseapee@gmail.com'}) unlocks full inquiry management and live portfolio publishing.
              </p>
              <button
                onClick={handleSignIn}
                disabled={isBusy}
                className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg bg-[#994524] hover:bg-[#7b2f0f] text-white text-sm font-semibold shadow-sm transition-all cursor-pointer disabled:opacity-50"
              >
            ...
            <>
              {/* Navigation Tabs (Admin can switch between Inquiries and Portfolio Manager) */}
              {isAdmin && (
                <div className="flex items-center gap-2 border-b border-[#e4e2df] pb-3">
                  <button
                  ...
                  {inquiries.length === 0 ? (
                    <div className="p-8 bg-white rounded-xl border border-[#e4e2df] text-center">
                      ...
                      <p className="text-xs text-[#55433c] mt-1">
                        Submitted inquiries from the contact section are stored in Firestore and appear here in real time.
                      </p>
                    </div>
                  ) : (
                    inquiries.map((inq) => (
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {!isAdmin && inq.status === 'new' && editingInquiryId !== inq.id && (
                              <button
                                onClick={() => {
                          ...
                          <div className="space-y-3 pt-2 border-t border-[#efeeeb]">
                            ...
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="px-3 py-1.5 text-xs text-[#55433c] hover:text-[#1b1c1a]"
                              >
                                Cancel
                              </button>
                              <button
                                ...
                                disabled={isBusy}
                                className="px-4 py-1.5 bg-[#994524] text-white text-xs font-semibold rounded-lg"
                              >
                                Save Changes
                              </button>
                            </div>
                    ...
                    {dynamicPortfolioItems.length === 0 ? (
                      <div className="p-8 bg-white rounded-xl border border-[#e4e2df] text-center">
                        <p className="font-serif text-base text-[#1b1c1a]">
                          No custom database portfolio entries yet
                        </p>
                        <p className="text-xs text-[#55433c] mt-1">
                          Entries added on the left are stored in Firestore and merged directly into your Selected Work showcase.
                        </p>
                      </div>
                    ) : (
